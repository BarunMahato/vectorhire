...
// 1. Updated import to grab authClient alongside useSession
...
function FileUploadZone({ 
  ...
  const { startUpload, isUploading } = useUploadThing("resumeUploader", {
    onClientUploadComplete: (res) => {
      ...
      // 2. Intercept raw CDN keys and force them into valid utfs.io URLs
      const safeUrl = uploaded.url.startsWith("http") 
        ? uploaded.url 
        : `https://utfs.io/f/${uploaded.key}`;

      onUploadComplete(safeUrl, fileName);
      setProgress(100);
    },
...
function OnboardingForm({ role }: { role: "STUDENT" | "RECRUITER" }) {
  const router = useRouter(); // 3. Added router to this component's scope
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (result.success) {
        // Refresh server components and move the user into the correct dashboard.
        router.refresh();
        router.push(role === "STUDENT" ? "/dashboard/student" : "/dashboard/recruiter");
      }
    } catch (error) {
  ...
  const fieldFocusClass = role === "RECRUITER" ? "focus:border-emerald-600" : "focus:border-blue-600";
  const selectedModeClass = role === "RECRUITER"
              ...
              <div>
                ...
                <div className="relative">
                  ...
                  <input
                    ...
                    className={`w-full p-4 pl-12 rounded-2xl bg-slate-50 border-2 border-transparent ${fieldFocusClass} focus:bg-white outline-none transition-all`}
            ...
            <motion.div key="s2" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} className="space-y-6">
              <div>
                ...
                <input
                  ...
                  className={`w-full p-4 rounded-2xl bg-slate-50 border-2 border-transparent ${fieldFocusClass} outline-none transition-all`}
                ...
                <div className="grid grid-cols-3 gap-3">
                  {["Remote", "Hybrid", "Onsite"].map((m) => (
                    <button
                      ...
                      className={`p-3 rounded-xl border-2 font-bold text-xs transition-all ${formData.workMode === m ? selectedModeClass : "border-slate-100 text-slate-400 hover:border-slate-200"}`}
                    >
                      {m}
                    </button>
                  ))}
...
function RoleButton({ onClick, emoji, title, desc, hoverColor }: RoleButtonProps) {
  return (
